...
const ICPLeadGenerator: React.FC = () => {
  ...
  const generateLeads = async () => {
    ...
    try {
      ...
      const prompt = `Generate ${leadCount} qualified B2B leads based on this Ideal Customer Profile:
      
      Company Profile:
      - Industries: ${selectedProfile.targetIndustries.join(', ')}
      - Company Size: ${selectedProfile.companySizeMin || 'Any'} - ${selectedProfile.companySizeMax || 'Any'} employees
      - Locations: ${selectedProfile.locations.join(', ')}
      - Technologies: ${selectedProfile.technologies.join(', ')}
      - Revenue Range: $${selectedProfile.revenueMin || 'Any'} - $${selectedProfile.revenueMax || 'Any'}
      
      Target Contacts:
      - Job Titles: ${selectedProfile.jobTitles.join(', ')}
      - Keywords: ${selectedProfile.keywords.join(', ')}
      - Exclusions: ${selectedProfile.exclusions.join(', ')}
      
      Description: ${selectedProfile.description}
      
      Please generate realistic leads with:
      ...
      Make sure all leads match the ICP criteria and are realistic, professional contacts.`
      
      const { object: generatedData } = await blink.ai.generateObject({
        prompt,
        schema: {
          type: 'object',
          properties: {
            leads: {
              ...
              items: {
                ...
                required: ['firstName', 'lastName', 'email', 'company', 'title', 'industry']
              }
            }
          },
          required: ['leads']
      ...
      const leadsToCreate = generatedData.leads.map(lead => ({
        ...lead,
        userId: user.id,
        score: Math.floor(Math.random() * 30) + 70, // High scores for AI-generated leads
        status: 'new' as const,
        source: 'ai_generated',
        generatedFromIcp: selectedProfile.name,
        icpProfileId: selectedProfile.id
      }))
      ...
      await blink.db.leadGenerationRequests.update(createdRequest.id, {
        status: 'completed',
        generatedCount: generatedData.leads.length,
        completedAt: new Date().toISOString()
      })
      
      toast({
        title: "Success!",
        description: `Generated ${generatedData.leads.length} qualified leads based on your ICP.`
      })
